...
export type SaraswatiFooterProps = {
  brandName?: string;
  brandLogoSrc?: string;
  nav?: NavSection[];
  social?: SocialLink[];
...
export default function Footer({
  ...
  legal = DEFAULT_LEGAL,
}: SaraswatiFooterProps) {
  const year = new Date().getFullYear();

  return (
    <footer className="bg-emerald-600 dark:bg-emerald-900 border-t border-border text-white dark:text-slate-100">
        ...
        <div className="grid gap-16 lg:grid-cols-[max-content_1fr] lg:gap-32">
          <div className="col-span-full flex max-w-max flex-col justify-between lg:col-span-1">
            <div>
              ...
              <p className="text-body-sm mt-3 text-white/80 dark:text-slate-200/80">
                {brandName} Technologies, LLC {year}
              </p>
            </div>

            <div className="mt-6 space-y-2">
              <p className="text-body-sm text-white/80 dark:text-slate-200/80">Follow us on:</p>
                ...
                {social.map((s) => {
                  ...
                  return (
                    <Link
                      ...
                      href={s.href}
                      aria-label={s.label}
                      className="bg-white/10 hover:bg-white/20 inline-flex size-9 items-center justify-center rounded-md border border-white/20 shadow-sm backdrop-blur-[2px] transition"
                      target="_blank"
                      rel="noreferrer"
                    >
          ...
          <div className="grid grid-cols-[repeat(auto-fit,minmax(160px,1fr))] gap-8 lg:grid-cols-3">
            {nav.map((section) => (
              <div key={section.title} className="space-y-4">
                <h4 className="text-body-md-medium text-white dark:text-slate-100">
                  {section.title}
                </h4>
                <ul className="space-y-3">
                  {section.links.map((l) => (
                    <li key={l.name}>
                      <Link
                        href={l.href}
                        className="text-body-md text-white/90 dark:text-slate-200/90 transition hover:text-white dark:hover:text-slate-100"
                      >
                        {l.name}
                      </Link>
                    </li>
          ...
          <div className="flex w-full flex-wrap items-center justify-center gap-x-6 gap-y-2">
            {legal.map((l) => (
              <Link
                key={l.name}
                href={l.href}
                className="text-body-sm text-white/80 dark:text-slate-200/80 transition hover:text-white dark:hover:text-slate-100"
              >
                {l.name}
              </Link>
            ))}
